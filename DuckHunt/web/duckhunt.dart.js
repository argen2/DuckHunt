...
Interceptor: {"": "Object;",
  ...
  "%": "ArrayBuffer|Blob|CanvasRenderingContext|CanvasRenderingContext2D|DOMError|DOMImplementation|File|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedNumberList"
},
...
["duckhunter", "duckhunter.dart", , M, {
main: function() {
  var t1, t2;
  $.driver = new M.Driver(new M.Start(0));
  t1 = $.get$ctx().canvas;
  t1.toString;
  ...
  C.Window_methods._requestAnimationFrame$1(t2, W._wrapZone(M.run$closure));
},

run: function(time) {
  var t1;
  M.update(time == null ? H.Primitives_getMilliseconds(P.DateTime$_now()) : time);
  M.render();
  t1 = window;
  C.Window_methods._ensureRequestAnimationFrame$0(t1);
  C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(M.run$closure));
},

update: function(time) {
  var t1, t2, t3, t4;
  if (J.$gt$n(J.$sub$n(time, $.lastUp), $.updateTimer)) {
    $.lastUp = H.Primitives_getMilliseconds(P.DateTime$_now());
    t1 = $.driver;
    t2 = $.eventClicked;
    t3 = $.eventClickedX;
    t4 = $.eventClickedY;
    t1.state = t1.state.update$3(t2, t3, t4);
    $.eventClicked = false;
  }
},

render: function() {
  $.get$ctx().clearRect(0, 0, 640, 400);
  var bg = document.querySelector("#bg");
  $.get$ctx().drawImage(bg, 0, 0);
  $.driver.state.render$0();
},
...
main_closure: {"": "Closure;",
  call$1: function(e) {
    var t1;
    $.eventClicked = true;
    $.eventClickedX = e.get$clientX();
    t1 = e.get$clientY();
    $.eventClickedY = t1.$sub(t1, 80);
  },
  $is_args1: true
},
...
Duck: {"": "Object;posX,posY,gotoX,gotoY,speed,repeat,loop,direction,number,isDead,isFinished,isFlyOff,directionChange",
  init$1: function(round) {
    this.speed = round;
    this.number = C.C__Random.nextInt$1(3) + 1;
    this.posX = C.C__Random.nextInt$1(600);
    this.posY = C.C__Random.nextInt$1(280);
    this.isDead = false;
  ...
  update$0: function() {
    ...
    if (C.JSInt_methods.$mod(t1, t2) === 0)
      if (this.isDead === true || this.isFlyOff === true)
  ...
  render$0: function() {
    var t1, pigeon, t2;
    if (this.isDead === true) {
      t1 = "#pigeon" + H.S(this.number) + "_dead";
      pigeon = document.querySelector(t1);
    } else {
      t1 = this.isFlyOff;
      t2 = this.number;
      if (t1 === true) {
        t1 = "#pigeon" + H.S(t2) + "_flyOff";
        pigeon = document.querySelector(t1);
      } else {
        t1 = "#pigeon" + H.S(t2) + "_" + this.direction;
        pigeon = document.querySelector(t1);
      }
    }
    if (pigeon != null)
      $.get$ctx().drawImage(pigeon, this.posX, this.posY);
  },
  flyOff$0: function() {
    this.gotoX = 0;
    this.gotoY = -6;
  ...
  isShot$2: function(x, y) {
    var t1;
    P.print("" + x + " " + y + " " + H.S(this.posX) + " " + H.S(this.posY));
    if (this.isDead !== true) {
      t1 = this.posX;
      if (typeof t1 !== "number")
        throw t1.$sub();
  ...
  changeDirection$0: function() {
    var t1, t2, x, y;
    t1 = Math.random();
    t2 = this.speed;
    if (typeof t2 !== "number")
      throw H.iae(t2);
    x = C.JSNumber_methods.toInt$0(Math.ceil(t1 * t2));
    t2 = Math.random();
    t1 = this.speed;
    if (typeof t1 !== "number")
      throw H.iae(t1);
    y = C.JSNumber_methods.toInt$0(Math.ceil(t2 * t1));
    this.loop = 0;
    this.repeat = C.C__Random.nextInt$1(10) + 10;
    if (x > 100)
      x = 100;
    if (y > 60)
      y = 60;
    ...
    } else if (C.C__Random.nextInt$1(2) === 1) {
      this.gotoX = -x;
      this.direction = "left";
    } else {
      this.gotoX = x;
    ...
    if (t1 + t2 > 280)
      ...
      this.gotoY = y;
    t1 = this.directionChange;
    if (typeof t1 !== "number")
      throw t1.$add();
    this.directionChange = t1 + 1;
    t1 = this.directionChange;
    if (typeof t1 !== "number")
      throw t1.$ge();
    if (t1 >= 20)
...
Gun: {"": "Object;round",
  init$0: function() {
    this.round = 3;
    J.set$innerHtml$x(document.querySelector("#shot"), "SHOT: " + H.S(this.round));
  },
  reloadIt$0: function() {
    this.round = 3;
    J.set$innerHtml$x(document.querySelector("#shot"), "SHOT: " + H.S(this.round));
...
Start: {"": "State;loop",
  update$3: function(clicked, clickedX, clickedY) {
    var res;
    this.loop = this.loop + 1;
    if (clicked) {
      res = new M.Play(false, 1, new M.Gun(null), new M.Duck(null, null, null, null, null, null, null, null, null, null, null, null, null), 0, 0, 0, 0, 0, 0, 0);
      res.update$3(false, clickedX, clickedY);
      return res;
    }
    return this;
  },
...
GameOver: {"": "State;",
  update$3: function(clicked, clickedX, clickedY) {
    var res;
    if (clicked) {
      J.set$display$x(document.querySelector("#play").style, "none");
      res = new M.Start(0);
      res.update$3(false, clickedX, clickedY);
      return res;
    }
  ...
  render$0: function() {
    ...
    $.get$ctx().drawImage(gameover, 0, 0);
  }
},

Play: {"": "State;isInit,round,gun,duck,pigeon,killedPigeon,score,posX,posY,gotoX,gotoY",
  init$0: function() {
    var i, t1;
    this.gun.init$0();
    this.duck.init$1(this.round);
    J.set$display$x(document.querySelector("#play").style, "block");
    this.isInit = true;
    document.querySelector("#h1").className = "hit active";
    for (i = 2; i <= 10; ++i) {
      t1 = "#h" + i;
      document.querySelector(t1).className = "hit";
    }
  },
  update$3: function(clicked, clickedX, clickedY) {
    var t1, t2, i, res;
    if (!this.isInit)
      this.init$0();
    if (clicked && this.gun.shoot$0()) {
      t1 = this.duck;
      P.print("SHOOT: " + clickedX + " " + clickedY + " " + H.S(t1.number) + " " + t1.direction);
      if (t1.isShot$2(clickedX, clickedY)) {
        t1 = this.score;
        t2 = this.gun.round;
        if (typeof t2 !== "number")
          throw t2.$add();
        this.score = t1 + 500 * (t2 + 1);
        this.killedPigeon = this.killedPigeon + 1;
        J.set$innerHtml$x(document.querySelector("#score"), "" + this.score + " points");
        t2 = "#h" + (this.pigeon + 1);
        J.set$innerHtml$x(document.querySelector(t2), "O");
      } else {
        t2 = this.gun.round;
        if (typeof t2 !== "number")
          throw t2.$le();
    ...
    if (t1.isFinished === true) {
      this.pigeon = this.pigeon + 1;
      t2 = "#h" + this.pigeon;
      document.querySelector(t2).className = "hit";
      t2 = this.pigeon;
      if (t2 < 10) {
        t2 = "#h" + (t2 + 1);
        document.querySelector(t2).className = "hit active";
      } else
        document.querySelector("#h1").className = "hit active";
      this.gun.reloadIt$0();
      if (this.pigeon === 10)
        if (this.killedPigeon >= 5) {
          this.round = this.round + 1;
          J.set$innerHtml$x(document.querySelector("#round"), "R" + this.round);
          this.pigeon = 0;
          this.killedPigeon = 0;
          for (i = 1; i <= 10; ++i) {
            t2 = "#h" + i;
            J.set$innerHtml$x(document.querySelector(t2), "X");
          }
        } else {
          res = new M.GameOver();
          res.update$3(false, clickedX, clickedY);
...
init.globalFunctions.run$closure = M.run$closure = new P.Closure$1(M.run, "run$closure");
...
//# sourceMappingURL=duckhunter.dart.js.map
//@ sourceMappingURL=duckhunter.dart.js.map
